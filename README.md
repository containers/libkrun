...
When exposing a directory in a filesystem from the host to the guest through virtio-fs devices configured with `krun_add_virtiofs` / `krun_add_virtiofs3`, libkrun **does not** provide any protection against the guest attempting to access other directories in the same filesystem, or even other filesystems in the host.
