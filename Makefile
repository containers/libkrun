...
FFIER_ALLOWS = \#![allow(dead_code, unused_unsafe, clippy::missing_safety_doc, clippy::needless_lifetimes)]

gen-init-blob-bindings:
	cargo build -p krun-init-blob-cdylib
	cargo run --bin krun-init-blob-gen c-header > $(LIBRARY_HEADER_INIT)
	{ echo '$(FFIER_ALLOWS)'; cargo run --bin krun-init-blob-gen rust-client; } > init/init-blob-via-cdylib/src/lib.rs
	{ echo '$(FFIER_ALLOWS)'; cargo run --bin krun-init-blob-gen rust-client --weak; } > init/init-blob-via-cdylib-weak/src/lib.rs
