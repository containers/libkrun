...
impl PassthroughFs {
    ...
    fn inode_to_handle(&self, inode: Inode) -> io::Result<InodeHandle> {
    ...
    fn open_inode(&self, inode: Inode, mut flags: i32) -> io::Result<File> {
        ...
        let ihandle = self.inode_to_handle(inode)?;
    ...
    fn do_getattr(&self, inode: Inode) -> io::Result<(bindings::stat64, Duration)> {
        ...
        let st = match ihandle {
            InodeHandle::VolPath(c_path) => lstat(&c_path, false)?,
    ...
    fn do_unlink(
        ...
        let ihandle = self.inode_to_handle(parent)?;
...
impl FileSystem for PassthroughFs {
    ...
    fn statfs(&self, _ctx: Context, inode: Inode) -> io::Result<bindings::statvfs64> {
        ...
        let res = match self.inode_to_handle(inode)? {
            InodeHandle::VolPath(c_path) => unsafe {
    ...
    fn setattr(
        ...
        let data = if let Some(handle) = handle {
            ...
            match self.inode_to_handle(inode)? {
                InodeHandle::VolPath(c_path) => Data::FilePath(c_path),
                InodeHandle::Fd(fd) => Data::Handle(fd),
            }
        };
        ...
        if valid.intersects(SetattrValid::ATIME | SetattrValid::MTIME) {
            ...
            let res = match data {
                ...
                Data::FilePath(ref c_path) => unsafe {
    ...
    fn link(
        ...
        let orig_c_path = match self.inode_to_handle(inode)? {
    ...
    fn access(&self, ctx: Context, inode: Inode, mask: u32) -> io::Result<()> {
        let st = match self.inode_to_handle(inode)? {
            InodeHandle::VolPath(c_path) => lstat(&c_path, false)?,
            InodeHandle::Fd(fd) => fstat(fd, false)?,
        };
