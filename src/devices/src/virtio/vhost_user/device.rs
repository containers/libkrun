...
impl VhostUserDevice {
    ...
    pub fn new(
        ...
        let actual_num_queues = if num_queues == 0 {
            // Query backend for queue count
            if backend_features & VHOST_USER_F_PROTOCOL_FEATURES != 0 {
                // Minimal protocol negotiation to query queue count
                frontend
                ...
                if protocol_features.contains(VhostUserProtocolFeatures::MQ) {
                    // Enable MQ to query queue count
                    frontend
                        .set_protocol_features(VhostUserProtocolFeatures::MQ)
                        .map_err(|e| io::Error::new(ErrorKind::Other, e))?;
                    ...
                    return Err(io::Error::new(
                        ErrorKind::InvalidInput,
                        "Backend doesn't support MQ protocol feature, must specify queue count",
                    ));
                }
            } else {
                return Err(io::Error::new(
            ...
            // User provided explicit queue count, use it
            num_queues as usize
        };
    ...
    fn activate_vhost_user(
        ...
        // If protocol features are supported and enabled, negotiate them now
        if self.backend_features & (1 << 30) != 0 {
            ...
            debug!(
                ...
                self.device_name, protocol_features
            );

            // We want CONFIG and MQ support at minimum
        ...
        for (queue_index, device_queue) in queues.iter().enumerate() {
            ...
            // Enable the vring (only if protocol features are supported)
    ...
    fn reset(&mut self) -> bool {
        ...
        if let Ok(mut frontend) = self.frontend.lock() {
            for queue_index in 0..self.queue_configs.len() {
                let _ = frontend.set_vring_enable(queue_index, false);
            }
        }
