use crate::{
    ...
    DisplayFeatures, DisplayVtable, ResourceFormat, header,
};
use log::{error, warn};
use std::ffi::c_void;
...
impl DisplayBackendBasicFramebuffer for DisplayBackendInstance {
    ...
    fn present_frame(&mut self, scanout_id: u32, frame_id: u32) -> Result<(), DisplayBackendError> {
        into_rust_result! {
            method_call!{
                self.present_frame(scanout_id, frame_id)
            }
        }
    }
}
...
impl<'a> DisplayBackend<'a> {
    pub fn create_instance(&self) -> Result<DisplayBackendInstance, DisplayBackendError> {
        ...
        assert_eq!(self.features, DisplayFeatures::BASIC_FRAMEBUFFER.bits());
    ...
    pub fn verify(&self) -> bool {
        for feature in DisplayFeatures::from_bits_retain(self.features) {
