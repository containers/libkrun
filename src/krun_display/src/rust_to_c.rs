use crate::{
    ...
    DisplayFeatures, DisplayVtable, DmabufExport, Rect, ResourceFormat,
};
use log::error;
use std::ffi::c_void;
...
pub fn into_display_backend_dmabuf<T: Sync, I: DisplayBackendDmabuf + DisplayBackendNew<T>>(
    ...
    extern "C" fn import_dmabuf_fn<I: DisplayBackendDmabuf>(
        ...
        match cast_instance::<I>(instance).import_dmabuf(dmabuf_export) {
            Ok(dmabuf_id) => dmabuf_id as i32,
            Err(e) => e as i32,
        }
    }

    extern "C" fn unref_dmabuf_fn<I: DisplayBackendDmabuf>(
        instance: *mut c_void,
        dmabuf_id: u32,
    ) -> i32 {
    ...
    extern "C" fn configure_scanout_dmabuf_fn<I: DisplayBackendDmabuf>(
        ...
        let src_rect: Option<&Rect> = unsafe { ptr_to_option_ref(src_rect) };
    ...
    DisplayBackend {
        ...
        vtable: DisplayVtable {
            dmabuf: DisplayDmabufVtable {
                ...
                import_dmabuf: Some(import_dmabuf_fn::<I>),
                unref_dmabuf: Some(unref_dmabuf_fn::<I>),
