...
fn build_rust_init() -> PathBuf {
    ...
    let default_cargo = std::env::var("CARGO").unwrap_or_else(|_| "cargo".to_string());
    let default_rustc = std::env::var("RUSTC").unwrap_or_else(|_| "rustc".to_string());
        ...
        None => {
            println!(
                "cargo:warning=musl target not available; krun-init will be dynamically linked. \
                 Run `rustup target add x86_64-unknown-linux-musl` for a static binary."
            );
    ...
    if std::env::var_os("TIMESYNC").map_or(false, |v| v == "1") {
