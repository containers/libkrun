...
pub fn setup_fpu(vcpu: &whp::WhpVcpu) -> Result<()> {
    let mut fp_cs: WHV_REGISTER_VALUE = unsafe { std::mem::zeroed() };
    fp_cs.FpControlStatus.Anonymous.FpControl = 0x37f;

    let mut xmm_cs: WHV_REGISTER_VALUE = unsafe { std::mem::zeroed() };
    xmm_cs.XmmControlStatus.Anonymous.XmmStatusControl = 0x1f80;

    vcpu.set_registers([
        (WHvX64RegisterFpControlStatus, fp_cs),
        (WHvX64RegisterXmmControlStatus, xmm_cs),
    ])
...
mod tests {
    ...
    #[test]
    fn test_setup_fpu_register_values() {
        // Verify setup_fpu constructs correct structured register values.
        // Actual WHP writes of FP registers (range 0x1000+) crash on some
        // Hyper-V builds, so we only verify the value construction here.
        let mut fp_cs: WHV_REGISTER_VALUE = unsafe { std::mem::zeroed() };
        fp_cs.FpControlStatus.Anonymous.FpControl = 0x37f;
        let fcw = unsafe { fp_cs.FpControlStatus.Anonymous.FpControl };
        assert_eq!(fcw, 0x37f);

        let mut xmm_cs: WHV_REGISTER_VALUE = unsafe { std::mem::zeroed() };
        xmm_cs.XmmControlStatus.Anonymous.XmmStatusControl = 0x1f80;
        let mxcsr = unsafe { xmm_cs.XmmControlStatus.Anonymous.XmmStatusControl };
