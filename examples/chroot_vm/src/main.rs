...
#[cfg(feature = "native")]
use krun::{BalloonDevice, ConsoleDevice, FsDevice, Init, RngDevice, VmmBuilder};
#[cfg(feature = "cdylib")]
use krun_cdylib::{BalloonDevice, ConsoleDevice, FsDevice, Init, RngDevice, VmmBuilder};
...
fn main() -> Result<()> {
    ...
    anyhow::ensure!(args.len() >= 3, "Usage: {} NEWROOT COMMAND [ARGS...]", args[0]);
    ...
    let payload = Init::builder(&rootfs, &mut console_builder)
        .exec(guest_cmd, &guest_args).context("exec")?
        .workdir("/").context("workdir")?
        .env(&["HOME=/root", "TERM=xterm-256color"]).context("env")?
        .build().context("build payload")?;
    ...
    let mut vmm = VmmBuilder::new()
        .vcpus(2).context("vcpus")?
        .ram_mib(512).context("ram")?
        .payload(payload)
        .device(rootfs)
        .device(console)
        .device(balloon)
        .device(rng)
        .build().context("build vmm")?;
    vmm.run();
    Ok(())
}
