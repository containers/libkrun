...
use crate::gtk_display::DisplayEvent;
...
impl ScanoutWindow {
    ...
    pub fn update(&mut self, buffer: Bytes) {
        self.scanout_paintable
            .update(buffer, self.width, self.height, self.format);
    }
}
...
impl DisplayWorker {
    pub fn new(app: Application, rx: PollableChannelReciever<DisplayEvent>) -> Self {
        let program_name = {
            ...
            DisplayEvent::ConfigureScanout {
                ...
                if let Some(ref mut scanout) = scanouts[scanout_id as usize] {
                    ...
                    scanouts[scanout_id as usize] = Some(ScanoutWindow::new(
                        &self.app,
                        &format!(
                            "{name} - gtk_display {scanout_id} ({width}x{height})",
                            name = self.app_name
                        ),
    ...
    /// Run a GTK application in the current thread handling the gtk_display events send over the channel.
    /// The events are produces by the `DisplayBackend` which is hooked up into libkrun.
    pub fn run(rx: PollableChannelReciever<DisplayEvent>) {
        ...
        let display_worker = Rc::new(DisplayWorker::new(app.clone(), rx));
