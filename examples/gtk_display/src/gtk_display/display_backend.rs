...
use krun_display::{
    DisplayBackendBasicFramebuffer, DisplayBackendError, DisplayBackendNew, ResourceFormat,
};
use log::error;
use std::mem;
...
// Implements libkrun traits (callbacks) to provide a gtk_display implementation, by forwarding the
...
impl DisplayBackendNew<PollableChannelSender<DisplayEvent>> for GtkDisplayBackend {
    fn new(channel: Option<&PollableChannelSender<DisplayEvent>>) -> Self {
        let channel = channel
            .expect("expected gtk_display struct to have valid userdata!")
            .clone();
...
impl DisplayBackendBasicFramebuffer for GtkDisplayBackend {
    fn configure_scanout(
        ...
        let required_buffer_size = width as usize * height as usize * BYTES_PER_PIXEL;
    ...
    fn present_frame(&mut self, scanout_id: u32, frame_id: u32) -> Result<(), DisplayBackendError> {
        ...
        self.channel
            .send(DisplayEvent::UpdateScanout { scanout_id, buffer })
            .unwrap();
        Ok(())
    }
}

fn resource_format_into_gdk(format: ResourceFormat) -> Result<MemoryFormat, DisplayBackendError> {
    Ok(match format {
