...
impl ScanoutWindow {
    ...
    pub fn new(
        ...
        attach_per_display_inputs(&picture, &overlay, window.as_ref(), per_display_inputs);
...
fn compute_point_inside_paintable(
    ...
    let p = overlay.compute_point(window, &Point::new(x as f32, y as f32))?;
    let p = picture.compute_point(overlay, &p)?;
    let point_x = p.x() as f64;
    let point_y = p.y() as f64;

    let px_rel = (point_x - x_space / 2.0) / (img_w - x_space);
    let py_rel = (point_y - y_space / 2.0) / (img_h - y_space);

    Some((px_rel.clamp(0.0, 1.0), py_rel.clamp(0.0, 1.0)))
}
        ...
        match options {
            DisplayInputOptions::TouchScreen(options) => {
                ...
                input_controller.connect_event(move |_, event| {
                    ...
                    let Some((x, y)) =
                        compute_point_inside_paintable(&picture, &overlay, &window, (x, y))
                    else {
                        return Propagation::Proceed;
                    };
                ...
                window.add_controller(input_controller);
            }
        }
    }
}
