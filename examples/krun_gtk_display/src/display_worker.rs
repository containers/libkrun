...
use krun_display::{DmabufExport, Rect};
...
use gtk::{
    ...
    EventControllerLegacy, EventControllerMotion, HeaderBar, Overlay, Picture,
    Revealer, RevealerTransitionType, Widget, Window,
    gdk::{
        self, EventSequence, EventType, MemoryFormat,
        ModifierType, TouchEvent,
    },
    gio::ActionEntry,
    gio::Cancellable,
    glib::{
...
impl ScanoutWindow {
    ...
    pub fn set_dmabuf(&mut self, dmabuf: SharedDmabuf, damage_area: Option<Rect>) {
        self.scanout_paintable.configure_dmabuf(dmabuf, None, damage_area);
    }
...
impl Drop for DmabufInner {
    fn drop(&mut self) {
        for &fd in self.0.dmabuf_fds.iter().take(self.0.n_planes as usize) {
            log::debug!("Closing dmabuf fd {} (fourcc=0x{:08x}, modifier=0x{:016x})",
...
impl SharedDmabuf {
    fn new(dmabuf_export: DmabufExport) -> Self {
        ...
        for &fd in dmabuf_export.dmabuf_fds.iter().take(dmabuf_export.n_planes as usize) {
