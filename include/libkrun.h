...
/**
 * Disable the implicit RNG device.
 *
 * By default, libkrun creates a virtio-rng device automatically. This function
 * disables that behavior. No implicit RNG device will be created.
 *
 * Note: Adding a vhost-user RNG via krun_add_vhost_user_device() also
 * automatically suppresses the implicit RNG device, so calling this function
 * is only necessary if you want no RNG device at all.
 *
 * Arguments:
 *  "ctx_id" - the configuration context ID.
 *
 ...
 *  Zero on success or a negative error number on failure.
 */
int32_t krun_disable_implicit_rng(uint32_t ctx_id);
