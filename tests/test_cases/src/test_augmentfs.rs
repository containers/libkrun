...
mod guest {
    ...
    impl Test for TestAugmentFs {
        fn in_guest(self: Box<Self>) {
            ...
            let dev_entries: Vec<_> = fs::read_dir("/dev")
                .expect("read_dir /dev")
                .collect();
            assert!(!dev_entries.is_empty(), "/dev listing should not be empty");

            let proc_entries: Vec<_> = fs::read_dir("/proc")
                .expect("read_dir /proc")
                .collect();
            assert!(!proc_entries.is_empty(), "/proc listing should not be empty");

            let sys_entries: Vec<_> = fs::read_dir("/sys")
                .expect("read_dir /sys")
                .collect();
